...
public class NumericEditText extends android.support.v7.widget.AppCompatEditText implements TextWatcher {
    ...
    public char mDecimalSeparator = DECIMAL_SEPARATOR;
    private boolean hasCustomDecimalSeparator = false;
    public boolean validateLock = false;
    int needSelectionPosition;
    public int prevLength;

    @Override
    public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        prevLength = getText().length();
        if(prevLength > 0) {
            needSelectionPosition =  prevLength - start;
        }
    }
    ...
    public void afterTextChanged(Editable s) {
        ...
        if(getText().length() < prevLength) {
                needSelectionPosition--;
        }
